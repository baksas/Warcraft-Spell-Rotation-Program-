...
AddFunction EnhancementAoeActions
{
	...
	#stormstrike
	Spell(stormstrike)
	#if hp low and talented ancestral_guidance_talent
	if {TalentPoints(ancestral_guidance_talent) and HealthPercent() < 20} Spell(ancestral_guidance)
...
AddFunction EnhancementSingleActions
{
	...
	if not TotemPresent(fire) Spell(searing_totem)
	#elemental_blast,if=talent.elemental_blast.enabled& swiftnes
	if TalentPoints(elemental_blast_talent) and {BuffPresent(ancestral_swiftness_buff) or Spell(ancestral_swiftness)} Spell(elemental_blast)
	#elemental_blast,if=talent.elemental_blast.enabled&buff.maelstrom_weapon.react>=1
	if TalentPoints(elemental_blast_talent) and BuffStacks(maelstrom_weapon_buff) >= 1 Spell(elemental_blast)
	#lightning_bolt,if=buff.maelstrom_weapon.react=5
	if {BuffStacks(maelstrom_weapon_buff) == 5 or BuffPresent(ancestral_swiftness_buff) or Spell(ancestral_swiftness)} Spell(lightning_bolt)
	#stormblast
	Spell(stormblast)
	#stormstrike
	Spell(stormstrike)
